...
export const login = async (email: string, password: string) => {
  try {
    const response = await axiosClient.post(`${API_BASE_URL}/auth/login`, { email, password });
...
export const refreshToken = async (refreshToken: string) => {
  try {
    const response = await axiosClient.post(`${API_BASE_URL}/refresh-token`, { refreshToken });
